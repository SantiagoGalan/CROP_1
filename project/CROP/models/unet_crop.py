...
class CropUnetBaseModel(ABC):
    ...
    def unmix(
        ...
        self.mixed_input = self._ensure_image(self.mixed_input)
        self.source1_estimation = self._ensure_image(self.source1_estimation)
        self.source2_estimation = self._ensure_image(self.source2_estimation)
    ...
    def reconstruction_by_condition(self,x_input):
        ...
        z_mean, z_log_var, z = self.unet.encoder.predict([x_repeated, one_hot_conditions])

        recons = self.unet.decoder.predict([z, one_hot_conditions])
