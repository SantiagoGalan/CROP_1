...
def vae_loss(y_true, y_pred):
  ...
  recon = y_pred[2]        # Imagen reconstruida por el decoder
  z_mean = y_pred[0]       # Media del espacio latente
  z_log_var = y_pred[1]    # Log-varianza del espacio latente
  ...
  mse = tf.keras.losses.MeanSquaredError()
  mse1 = mse(y_true, y_pred) * tf.cast(tf.reduce_prod(tf.shape(y_true)[1:]), tf.float32)  # Número de píxeles dinámico
