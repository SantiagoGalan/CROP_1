...
def best_digit_var_sigmoid(
    x_mix_filtrado_2, x_mix_orig, alpha, bias, slope, cvae, predictor, show_laten=False
):

    # First decoded image --------------------------------------------------------------
    x_mix_filtrado_1 = 2 * x_mix_orig - x_mix_filtrado_2  # Masked (Cochlear) x'2
    x_mix_filtrado_1 = tf.clip_by_value(
        x_mix_filtrado_1, clip_value_min=0, clip_value_max=1
    )  # Filtered mix
    condition_encoder = predictor.predict(
        x_mix_filtrado_1, verbose=0
    )  # * j * alfa     # con ponderado incremental
    condition_decoder_1 = condition_encoder

    encoded_imgs = cvae.encoder.predict(
        [x_mix_filtrado_1, condition_encoder], verbose=0
    )
